  ...
  static async update(req: Request, res: Response) {
    ...
    try {
      ...
      if (!updateChar) {
        ...
        return res
          .status(500)
          .json({ message: "Internal Server Error updating character" });
      }
  ...
  static async delete(req: Request, res: Response) {
    ...
    try {
      ...
      if (!deleteChar) {
        ...
        return res
          .status(500)
          .json({ message: "Internal Server Error deleting character" });
      }
      ...
      //Return message
      res.json({
        message: `Character '${findCharacter.name}' with id of '${id}' successfully deleted.`,
      });
    } catch (error) {
      //ROLLBACK
      await t.rollback();
